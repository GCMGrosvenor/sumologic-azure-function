...
param($Sdk = "Gcm.MSBuild.Sdk", $SdkVersion = "0.2.*", $SdkInstallArgs, $DotNetSdkVersion, [Alias('u')]$UpdateSdk = $true, [Alias('c')]$Configuration = 'Release', [Alias('o')]$Output = "$PSScriptRoot/../artifacts", [Alias('v')]$Verbosity = 'n', [Parameter(ValueFromRemainingArguments)][array]$CommandArgs)
